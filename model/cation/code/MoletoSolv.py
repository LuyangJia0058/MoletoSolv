...
def getIntitialDataSet(strPath):
    ...
    strFilePath = BASISDIR + "dataset.csv"
...
def splitDataSet(arrRawData, arrRawLabel, arrRawFeatures, strPath):
    ...
    arrWaterDataIndices = arrRawGroupedData.get_group(ClassificationData).index      
    arrWaterTrainData, arrWaterTestData = train_test_split(arrWaterData, test_size=0.2, random_state=nRandomSeed)
    
    ClassificationData=float(35.6881) 
    arrAceData = arrRawGroupedData.get_group(ClassificationData)
    arrAceDataIndices = arrRawGroupedData.get_group(ClassificationData).index     
    arrAceTrainData, arrAceTestData = train_test_split(arrAceData, test_size=0.2, random_state=nRandomSeed)

    ClassificationData=float(46.826) 
    arrDiData = arrRawGroupedData.get_group(ClassificationData)
    arrDiDataIndices = arrRawGroupedData.get_group(ClassificationData).index     
    arrDiTrainData, arrDiTestData = train_test_split(arrDiData, test_size=0.2, random_state=nRandomSeed)

    
    ClassificationData=float(32.613) 
    arrMeData = arrRawGroupedData.get_group(ClassificationData)
    arrMeDataIndices = arrRawGroupedData.get_group(ClassificationData).index      
    arrMeTrainData, arrMeTestData = train_test_split(arrMeData, test_size=0.2, random_state=nRandomSeed)
      

    arrRawTrainData = pd.concat([arrWaterTrainData, arrAceTrainData, arrMeTrainData, arrDiTrainData])
    arrRawTestData = pd.concat([arrWaterTestData, arrAceTestData, arrMeTestData, arrDiTestData])
    ...
    print(f"Train set size: {arrRawTrainData.shape[0]}")
    print(f"Test set size: {arrRawTestData.shape[0]}")
