...
def splitDataSet(arrRawData, arrRawLabel, arrRawFeatures, strPath):
    ...
    print("splitDataSet")
    nRandomSeed = 8
    print(f"Random seed for splitting: {nRandomSeed}")
    ...
    for _, eps in solvent_eps.items():
        group_data = arrRawGroupedData.get_group(float(eps))
        train, test = train_test_split(group_data, test_size=0.2, random_state=nRandomSeed)
        train_parts.append(train)
        test_parts.append(test)

    arrRawTrainData = pd.concat(train_parts).to_numpy(dtype=np.float64)
    arrRawTestData = pd.concat(test_parts).to_numpy(dtype=np.float64)
